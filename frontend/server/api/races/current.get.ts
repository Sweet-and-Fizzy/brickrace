...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    const { data: nextHeats, error: nextHeatsError } = await client
      .rpc('get_next_heats', { heat_count: 2 })
    ...
    const upcomingHeats = validNextHeats.length > 0 ? validNextHeats.reduce((acc: any, heat: any) => {
    ...
    const upcomingHeatsArray = Object.values(upcomingHeats)
      .sort((a: any, b: any) => a.scheduled_order - b.scheduled_order)

    return {
      data: {
        race: activeRace,
        currentHeat: currentHeat && currentHeat.length > 0 ? {
